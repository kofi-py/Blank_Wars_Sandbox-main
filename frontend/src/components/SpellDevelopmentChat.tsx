...
import { Send, Wand2, Star, Sparkles } from 'lucide-react';
...
interface SpellDevelopmentChatProps {
...
export default function SpellDevelopmentChat({
  selected_characterId,
  onCharacterChange,
  selected_character,
  available_characters,
  coach_name
}: SpellDevelopmentChatProps) {
  ...
  // Generate quick messages for spell development
  const quickMessages = [
    "What spells should I prioritize?",
    "How should I spend my character points on magic?",
    "Which spell tier should I focus on?",
    "Should I unlock new spells or rank up existing ones?",
    "What spells fit my magical style?"
  ];

  const sendMessage = async (content: string) => {
    if (!content.trim() || isTyping || !selected_character) return;
    if (!selected_character.name) {
      throw new Error('STRICT MODE: selected_character.name is required for spells chat');
    }
    if (!selected_character.character_id) {
      throw new Error('STRICT MODE: selected_character.character_id is required for spells chat');
    }
    ...
    try {
      const chat_id = `spells_${selected_character.id}`;
  ...
  return (
    <div className="bg-gradient-to-r from-indigo-900/30 to-purple-900/30 rounded-xl p-6 border border-indigo-500/30 mb-6">
      {/* Header */}
      <div className="mb-4">
        ...
        <p className="text-gray-400 text-sm">
          Discuss spell choices and magical development with {selected_character?.name || 'your character'}
        </p>
      </div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {quickMessages.map((msg, idx) => (
          <button
            ...
            disabled={isTyping}
            className="px-3 py-1.5 text-sm bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {msg}
          </button>
        ))}
      ...
      <div className="bg-gray-900/50 rounded-lg p-4 mb-4 h-64 overflow-y-auto">
        {messages.length === 0 && (
          <div className="text-center text-gray-500 py-8">
            ...
            <p>Start a conversation about spell development</p>
          </div>
        )}
            ...
            >
              <div className="flex items-start gap-2">
                {message.type === 'contestant' && (
                  <span className="text-2xl">{selected_character?.avatar || '✨'}</span>
                )}
                <div>
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder="Ask about spell development..."
          disabled={isTyping}

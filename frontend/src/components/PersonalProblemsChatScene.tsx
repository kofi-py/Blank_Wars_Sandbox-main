...
/**
 * PersonalProblemsChatScene - Full-screen immersive 3D personal problems coaching
 *
 ...
 * - Coach input as overlay at bottom
 */
...
// Problem categories for generating character-specific challenges
...
// Generate character-specific personal problem
const generatePersonalProblem = (character: Contestant): { problem: string; intro: string } => {
    const { name, archetype } = character;
    const randomProblem = PROBLEM_CATEGORIES[Math.floor(Math.random() * PROBLEM_CATEGORIES.length)];

    const problemIntros: Record<string, string> = {
        neighbor_disputes: `Coach, I'm having issues with people in my life who keep challenging my boundaries...`,
        family_conflicts: `My family doesn't understand my choices. We're growing apart and I don't know how to fix it...`,
        work_stress: `The constant pressure is wearing me down. Every day feels like an uphill battle...`,
        relationship_issues: `I care deeply for someone, but my lifestyle keeps creating problems between us...`,
        financial_problems: `Money stress is consuming me. I'm struggling to see a way forward...`,
        health_concerns: `I'm worried about my health. My body is giving me warning signs I can't ignore...`,
        identity_crisis: `I'm lost, Coach. I don't know who I am anymore or what I'm supposed to be doing...`,
        authority_conflicts: `I keep clashing with authority figures. I can't seem to play by their rules...`,
        moral_dilemmas: `I'm facing an impossible moral choice. Every option feels wrong but doing nothing is worse...`,
        social_isolation: `I feel completely alone. I'm surrounded by people but no one really understands me...`,
        trust_issues: `I can't trust anyone anymore. Past betrayals have made me paranoid and defensive...`,
        anger_management: `My temper is ruining everything good in my life. I don't know how to control it...`,
        perfectionism: `Nothing I do ever feels good enough. My perfectionism is crippling me...`,
        impostor_syndrome: `I feel like a complete fraud. Everyone thinks I'm competent but I'm just fooling them...`,
        grief_loss: `I lost someone important to me. The grief is consuming everything and I don't know how to heal...`
    };

    return {
        problem: randomProblem,
        intro: problemIntros[randomProblem] || `Coach, I'm dealing with some personal challenges and could use your guidance.`
    };
};

export interface PersonalProblemsChatSceneProps {
    availableCharacters: Contestant[];
    onClose?: () => void;
}

export interface PersonalProblemsChatSceneRef {
    speak: (sentences: string[]) => void;
}

const PersonalProblemsChatScene = forwardRef<PersonalProblemsChatSceneRef, PersonalProblemsChatSceneProps>(({
    availableCharacters,
    onClose
}, ref) => {
    // Blue/Purple theme for personal problems
    ...
    const handleStartSession = async (character: Contestant) => {
        ...
        const newSessionId = `personal_problems_${character.id}_${Date.now()}`;
        setSessionId(newSessionId);

        // Generate a personal problem for this character
        const problem = generatePersonalProblem(character);
        ...
        try {
            ...
            const response = await sendChat(sessionId, {
                ...
                meta: {
                    ...
                    personal_problem: currentProblem,
            ...
            if (response?.text) {
                ...
                try {
                    ...
                    await eventBus.publish({
                        ...
                        description: `${activeCharacter.name} in personal problems session`,
                        metadata: { session_type: 'personal_problems', user_message: content, ai_response: response.text },
                        tags: ['personal_problems', 'coaching']
                    });
                } catch (e) { console.error('Event publish error:', e); }
            }
        } catch (error) {
            console.error('Personal problems chat error:', error);
    ...
    return (
        <div ref={containerRef} style={{ width: '100%', height: '100%', background: 'linear-gradient(to bottom, #1e1b4b, #1e3a5f)', position: 'relative', overflow: 'hidden' }}>
            {/* Header */}
            <div style={{
                position: 'absolute',
                top: 20,
                ...
                <h1 style={{ color: 'white', fontFamily: 'sans-serif', fontSize: '24px', margin: 0 }}>
                    Personal Problems Coaching
                </h1>
                {activeCharacter && (
                    <div style={{ color: '#60a5fa', fontSize: '14px', marginTop: 4 }}>
                        Session with {activeCharacter.name}
                    </div>
                )}
            </div>
            ...
            {!characterConfig && (
                <div style={{
                    ...
                    <p style={{ color: '#888', marginTop: 10 }}>Choose someone to begin a personal problems coaching session</p>
                </div>
            )}
...
PersonalProblemsChatScene.displayName = 'PersonalProblemsChatScene';

export default PersonalProblemsChatScene;

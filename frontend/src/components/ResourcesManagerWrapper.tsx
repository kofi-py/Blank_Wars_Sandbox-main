...
interface ResourcesManagerWrapperProps {
...
export default function ResourcesManagerWrapper({
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.error('Failed to load characters for resources manager:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!selected_character) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-center text-gray-400">No characters found for resources.</div>
      </div>
    );
  }

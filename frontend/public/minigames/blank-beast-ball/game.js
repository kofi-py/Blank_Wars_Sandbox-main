...
let isPlayerJumping = false; // Track when player initiates a jump (for animation)
let obstacles = [];
let keys = {};
let gameState = 'menu';
...
function loadCharacterCards() {
    ...
    // Create cards for each character - using event delegation (no inline onclick to avoid double-firing)
    let cardsHTML = '';
...
function setupEventListeners() {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        // Prevent arrow keys and WASD from scrolling when game is active
...
let isGameStarting = false; // Guard against double initialization
...
async function createPlayer() {
    // Try to load GLB model for Achilles, otherwise use fallback
    if (modelLoader && selectedCharacter.id === 'achilles') {
        try {
            console.log('Loading Achilles GLB model...');
            player = await modelLoader.createPlayerCharacter('achilles', selectedCharacter, scene);
            player.position.set(0, PLAYER_SIZE, 0);
            // Rotation applied in model-loader.js
            playerVelocity = new THREE.Vector3(0, 0, 0);
            playerOnGround = false;
            isPlayerJumping = false;
            console.log('✅ Achilles model loaded!');
            return;
        } catch (error) {
            console.error('Failed to load Achilles model:', error);
...
function jump() {
    if (playerOnGround) {
        ...
        // Hold a static airborne pose (freeze at 40% of jump animation)
        if (modelLoader && player && player.userData.actions?.jump) {
            const currentAction = player.userData.currentAction;
            const jumpAction = player.userData.actions.jump;
            if (currentAction) currentAction.stop();
            jumpAction.reset();
            jumpAction.time = jumpAction.getClip().duration * 0.4; // Freeze at good airborne pose
            jumpAction.paused = true; // Don't animate, just hold the pose
            jumpAction.play();
            player.userData.currentAction = jumpAction;
            player.userData.currentAnimState = 'jump';
        }
    }
}
...
function updatePlayer() {
    ...
    // End jump animation when player starts falling
    if (isPlayerJumping && playerVelocity.y < 0) {
        console.log('🔻 Jump ended - velocity negative:', playerVelocity.y);
        isPlayerJumping = false;
    }

    // Update player animation based on movement
    // Use isPlayerJumping flag (set when space pressed) instead of just being in air
    if (modelLoader && player) {
        // Log when jump animation should trigger
        if (isPlayerJumping) {
            console.log('🎬 Sending jump animation state, velocity:', playerVelocity.y);
        }
        modelLoader.updatePlayerAnimation(player, isMoving, isPlayerJumping);
    }
    ...
    for (let obstacle of obstacles) {
        ...
        if (player.position.x > actualX - bounds.width / 2 &&
            ...
            if (player.position.y - PLAYER_SIZE <= platformTop &&
                ...
                playerVelocity.y = 0;
                playerOnGround = true;
                isPlayerJumping = false; // Landed on platform, end jump animation
...
function endGame(won, message = '') {
    ...
    if (won) {
        ...
        // Store characterId before resetGame clears selectedCharacter
        const currentCharacterId = Object.keys(BLANK_WARS_CHARACTERS).find(key =>
...
function resetGame() {
    // Reset the starting guard so game can be started again

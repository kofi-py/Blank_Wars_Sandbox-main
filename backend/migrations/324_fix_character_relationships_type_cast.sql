-- Migration 324: Fix character_relationships type cast in get_full_character_data
--
-- PROBLEM: character_relationships table has varchar columns but function compares to uuid:
--   - cr.character2_id (varchar) compared to uc2.id (uuid) at line 374
--   - cr.character1_id (varchar) compared to p_userchar_id (uuid) at line 376
--
-- FIX: Cast uuid values to text for comparison

BEGIN;

DO $$
BEGIN
    RAISE NOTICE 'Migration 324: Fixing character_relationships type cast...';
END $$;
...
BEGIN
  ...
  -- FIX: Cast uuid to text for comparison with varchar columns
  SELECT jsonb_agg(jsonb_build_object(
...
DO $$
BEGIN
    RAISE NOTICE 'Migration 324 complete: Fixed character_relationships type cast (uuid::text)';
END $$;

-- Log migration
INSERT INTO migration_log (version, name)
VALUES (324, '324_fix_character_relationships_type_cast')

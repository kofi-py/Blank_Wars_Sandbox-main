-- Migration 277: Fix jsonb_agg NULL handling with STRICT MODE
--
-- Problem: jsonb_agg() returns NULL when no rows match. TypeScript expects arrays.
--
-- Solution:
-- 1. recent_memories/recent_decisions: COALESCE to empty array (valid for new characters)
-- 2. roommates/teammates/powers/spells/equipment/items/relationships/category_preferences:
--    These MUST have starting values. Throw STRICT MODE error if NULL.
...
BEGIN
  ...
  -- inventory: MUST have values (starter equipment)
  SELECT jsonb_agg(jsonb_build_object(
    'equipment_id', ce.equipment_id,
    'is_equipped', ce.is_equipped,
    'item_name', e.name,
  ...
  IF v_inventory IS NULL THEN
    RAISE EXCEPTION 'STRICT MODE: User character % has no inventory', p_userchar_id;
  END IF;

  -- items: CAN be empty (consumables are optional)
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', ci.item_id,
    'name', i.name,
  ...
  -- powers: MUST have values (starter powers)
  SELECT jsonb_agg(jsonb_build_object(
  ...
  IF v_powers IS NULL THEN
    RAISE EXCEPTION 'STRICT MODE: User character % has no unlocked powers', p_userchar_id;
  END IF;

  -- spells: MUST have values (starter spells)
  SELECT jsonb_agg(jsonb_build_object(
  ...
  IF v_spells IS NULL THEN
    RAISE EXCEPTION 'STRICT MODE: User character % has no unlocked spells', p_userchar_id;
  END IF;

  -- category_preferences: MUST have values (seeded on character creation)
  SELECT jsonb_agg(jsonb_build_object(
  ...
  IF v_category_preferences IS NULL THEN
    RAISE EXCEPTION 'STRICT MODE: User character % has no category preferences', p_userchar_id;
  END IF;

  -- relationships: MUST have values (created with teammates)
  SELECT jsonb_agg(jsonb_build_object(
...
VALUES (277, '277_fix_jsonb_agg_nulls_strict_mode')

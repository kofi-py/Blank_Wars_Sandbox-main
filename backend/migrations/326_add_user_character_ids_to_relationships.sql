-- Migration 326: Add user_character_id columns to character_relationships
--
-- PROBLEM: character_relationships stores base character IDs (e.g., 'space_cyborg')
-- but relationships should be per-user/per-team, not shared globally.
-- The gameEventBus passes user_character UUIDs but queries match against base IDs.
--
-- FIX: Add user_character1_id and user_character2_id columns to store UUIDs
-- This allows per-user relationships instead of shared canonical values.

BEGIN;

DO $$
BEGIN
    RAISE NOTICE 'Migration 326: Adding user_character_id columns to character_relationships...';
END $$;

-- Add user_character1_id column
ALTER TABLE character_relationships
ADD COLUMN IF NOT EXISTS user_character1_id UUID REFERENCES user_characters(id) ON DELETE CASCADE;

-- Add user_character2_id column
ALTER TABLE character_relationships
ADD COLUMN IF NOT EXISTS user_character2_id UUID REFERENCES user_characters(id) ON DELETE CASCADE;

-- Add unique constraint for user_character pairs (allows NULL for legacy data)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'character_relationships_user_char_pair_unique'
    ) THEN
        ALTER TABLE character_relationships
        ADD CONSTRAINT character_relationships_user_char_pair_unique
        UNIQUE (user_character1_id, user_character2_id);
    END IF;
END $$;

-- Add index for lookups by user_character1_id
CREATE INDEX IF NOT EXISTS idx_character_relationships_user_char1
ON character_relationships (user_character1_id)
WHERE user_character1_id IS NOT NULL;

-- Add index for lookups by user_character2_id
CREATE INDEX IF NOT EXISTS idx_character_relationships_user_char2
ON character_relationships (user_character2_id)
WHERE user_character2_id IS NOT NULL;

-- Update get_full_character_data to query by user_character1_id
...
BEGIN
  ...
  FROM character_equipment ce
  JOIN equipment e ON ce.equipment_id = e.id
  WHERE ce.character_id = p_userchar_id AND ce.is_equipped = false;
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'power_name', p.name,
    'power_level', cp.current_rank
  )), '[]'::jsonb)
  INTO v_powers
  FROM character_powers cp
  JOIN power_definitions p ON cp.power_id = p.id
  WHERE cp.character_id = p_userchar_id;

  -- spells: CAN be empty (depends on character build)
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'spell_id', cs.spell_id,
    'spell_name', s.name,
    'spell_level', cs.current_rank
  )), '[]'::jsonb)
  INTO v_spells
  FROM character_spells cs
  JOIN spell_definitions s ON cs.spell_id = s.id
  WHERE cs.character_id = p_userchar_id;

  -- category_preferences: CAN be empty for new characters
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'category_type', cp.category_type,
    'category_value', cp.category_value,
  ...
  -- relationships: Query by user_character1_id (new column)
  -- Returns empty array if no relationships exist yet (new characters)
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'user_character_id', cr.user_character2_id,
    'character_id', uc2.character_id,
    'name', c.name,
  ...
  JOIN user_characters uc2 ON cr.user_character2_id = uc2.id
  JOIN characters c ON uc2.character_id = c.id
  WHERE cr.user_character1_id = p_userchar_id;
...
DO $$
BEGIN
    RAISE NOTICE 'Migration 326 complete: Added user_character_id columns to character_relationships';
END $$;

-- Log migration
INSERT INTO migration_log (version, name)
VALUES (326, '326_add_user_character_ids_to_relationships')

-- Migration 232: Add comprehensive STRICT MODE checks to get_full_character_data
-- Previously only checked 5 combat stats - now checks all critical fields
-- This helps catch missing data during development
...
BEGIN
  ...
  v_identity := jsonb_build_object(
    ...
    'recent_opponents', (
      ...
      JOIN user_characters uc ON bp.character_id = uc.id
      JOIN characters c ON uc.character_id = c.id
      WHERE b.id IN (
        SELECT battle_id FROM battle_participants WHERE character_id = p_userchar_id
      )
...
COMMENT ON FUNCTION get_full_character_data(TEXT) IS
'Returns complete character data as JSONB in 3 packages: IDENTITY, COMBAT, PSYCHOLOGICAL.
STRICT MODE: Raises exception if user_character not found or ANY critical field missing.
Now checks both combat stats AND psychological stats (stress, confidence, morale, etc.).
Parameter: p_userchar_id (user_characters instance UUID)
The canonical character_id is derived internally from user_characters.character_id
and returned in IDENTITY.character_id for downstream persona lookup.
NOTE: For system characters (therapists, judges), use fetchSystemCharacterData() in TypeScript

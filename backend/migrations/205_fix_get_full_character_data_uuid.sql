-- Migration 205: Fix UUID type mismatch in get_full_character_data function
-- 
-- Issue: The function incorrectly casts teams.id to text when comparing with team_context.team_id
-- History:
--   - Migration 024 created team_context with team_id VARCHAR(255)
...
-- 
-- Fix: Remove the ::text cast since both columns are now UUID
...
BEGIN
  ...
  -- Both team_context.team_id and teams.id are UUID (fixed in migration 042)
  -- =====================================================
  SELECT tc.*
  INTO v_tc

-- Migration 323: Fix get_full_character_data column name errors
--
-- PROBLEM: Migration 321 incorrectly used 'user_character_id' for tables that use 'character_id':
--   - character_equipment (lines 294, 307)
--   - character_items (line 318)
--   - character_powers (line 329)
--   - character_spells (line 340)
--   - character_category_preferences (line 351)
--
-- Only character_memories actually has a user_character_id column.
--
-- FIX: Change user_character_id to character_id for all affected tables

BEGIN;

DO $$
BEGIN
    RAISE NOTICE 'Migration 323: Fixing column name errors in get_full_character_data...';
END $$;
...
BEGIN
  ...
  -- NOTE: character_memories uses user_character_id (correct)
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'content', cm.content,
  ...
  -- FIX: Changed user_character_id to character_id
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
  ...
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'name', c.name,
    'affection', cr.affection,
    'rivalry', cr.rivalry,
    'trust', cr.trust,
    'familiarity', cr.familiarity
  ) ORDER BY c.name)
  INTO v_relationships
  FROM character_relationships cr
  JOIN user_characters uc2 ON cr.character2_id = uc2.id
  JOIN characters c ON uc2.character_id = c.id
  WHERE cr.character1_id = p_userchar_id;
...
DO $$
BEGIN
    RAISE NOTICE 'Migration 323 complete: Fixed 6 column name errors (user_character_id -> character_id)';
END $$;

-- Log migration
INSERT INTO migration_log (version, name)
VALUES (323, '323_fix_get_full_character_data_column_names')

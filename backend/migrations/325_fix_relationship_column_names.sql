-- Migration 325: Fix character_relationships column names in get_full_character_data
--
-- PROBLEM: Function uses wrong column names for character_relationships:
--   - cr.affection should be cr.current_affection
--   - cr.rivalry should be cr.current_rivalry
--   - cr.trust should be cr.current_trust
--   - cr.familiarity does not exist (removed)
--
-- Verified against schema: \d character_relationships

BEGIN;

DO $$
BEGIN
    RAISE NOTICE 'Migration 325: Fixing character_relationships column names...';
END $$;
...
BEGIN
  ...
  -- FIX: Use correct column names from schema (current_affection, current_rivalry, current_trust)
  -- Removed familiarity (column does not exist)
  SELECT jsonb_agg(jsonb_build_object(
  ...
  JOIN user_characters uc2 ON cr.character2_id = uc2.id::text
  JOIN characters c ON uc2.character_id = c.id
  WHERE cr.character1_id = p_userchar_id::text;
...
DO $$
BEGIN
    RAISE NOTICE 'Migration 325 complete: Fixed character_relationships column names';
END $$;

-- Log migration
INSERT INTO migration_log (version, name)
VALUES (325, '325_fix_relationship_column_names')

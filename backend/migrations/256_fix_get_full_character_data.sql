-- Migration 256: Fix get_full_character_data after UUID migration
-- Purpose:
--   1. Fix battles.created_at → battles.started_at (column doesn't exist)
--   2. Fix character_memories query to use user_character_id (not character_id)
--   3. Remove unnecessary ::uuid casts (columns are now UUID type)
-- Context: Migration 255 converted TEXT columns to UUID, making casts redundant
  ...
  v_identity := jsonb_build_object(
    ...
    'recent_decisions', (
      ...
      FROM (
        ...
        -- FIXED: Removed ::uuid cast (column is now UUID type)
    ...
    'recent_memories', (
      ...
      FROM (
        ...
        -- FIXED: Changed from character_id to user_character_id
        -- character_id is character varying (canonical names like "dracula")
        -- user_character_id is UUID (references user_characters.id)
    ...
    'recent_opponents', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        -- FIXED: Changed from b.created_at to b.started_at
        -- battles table has started_at and ended_at, not created_at
...
VALUES (256, '256_fix_get_full_character_data')
...
-- ============================================================================
-- Test get_full_character_data (should work without errors now)

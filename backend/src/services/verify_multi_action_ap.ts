...
async function runMultiActionAPTest(): Promise<void> {
    console.log('═══════════════════════════════════════════════════════');
    console.log('🎯 MULTI-ACTION AP SYSTEM TEST');
    ...
    // =============================================
    console.log('📋 TEST 1: Verify Attack Types...');
    try {
        const attack_types = await query(`
        ...
        results.push({
            ...
            details: attacks_valid
                ? 'jab=1AP, strike=2AP, heavy=3AP ✓'
                : `Expected jab = 1, strike = 2, heavy = 3. Got: jab = ${ jab?.ap_cost }, strike = ${ strike?.ap_cost }, heavy = ${ heavy?.ap_cost } `
        });
        console.log(attacks_valid ? '   ✅ Attack types verified\n' : '   ❌ Attack types incorrect\n');
    ...
    // =============================================
    console.log('📋 TEST 2: Verify AP Combinations...');
    const combinations = [
    ...
    for (const combo of combinations) {
        ...
        console.log(`   ${ combo_correct ? '✓' : '✗' } ${ combo.desc }: ${ can_execute ? 'CAN' : 'CANNOT' } execute`);
    }
    ...
    console.log('   ✅ AP combination logic verified\n');

    // =============================================
    // TEST 3: Test Actual Battle Turn with Multi-Actions
    // =============================================
    console.log('📋 TEST 3: Create Battle and Test Multi-Action Turn...');
    ...
    `, [user_a_id, user_b_id]);
        ...
        // =============================================
        console.log('\n📋 TEST 4: Record Multi-Action Turn...');
    ...
    `, [test_battle_id, char.id]);
        ...
        console.log(actions.rows.length === 3 ? '   ✅ Multi-action turn recorded\n' : '   ❌ Recording failed\n');

        // =============================================  
        // TEST 5: Test Mixed Action Turn (strike + defend)
        // =============================================
        console.log('📋 TEST 5: Mixed Action Turn (strike + defend = 3AP)...');
    ...
    `, [test_battle_id, char2.id, turn_number_2]);
        ...
        console.log(mixed_ap === 3 ? '   ✅ Mixed action turn verified\n' : '   ❌ Mixed action failed\n');

        // =============================================
        // TEST 6: Heavy Attack (uses all 3 AP)
        // =============================================
        console.log('📋 TEST 6: Heavy Attack (3 AP, uses entire turn)...');
    ...
    `, [
      ...
      WHERE battle_id = $1 AND character_id = $2 AND turn_num = $3
        ...
        console.log(heavy_result?.ap_cost === 3 ? '   ✅ Heavy attack verified\n' : '   ❌ Heavy attack failed\n');

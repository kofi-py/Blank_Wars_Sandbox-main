/**
 * Message Board Personas Index
 * Maps character_id to persona builder for all 33 contestants.
 *
 * NOTE: Only contestants have messageBoard personas - system characters
...
export default function getPersona(
  ...
  if (!personaBuilder) {
    throw new Error(`STRICT MODE: Message board persona not found for character "${characterId}". Only contestants have message board personas. Available: ${Object.keys(personas).join(', ')}`);
  }

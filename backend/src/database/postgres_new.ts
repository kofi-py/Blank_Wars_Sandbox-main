...
async function fallbackBasicInitialization(): Promise<void> {
  ...
  try {
    ...
    await query(`
      ...
      VALUES ('000_basic_fallback', 'Basic fallback initialization - please run full migrations ASAP')
...
// Cache service (can be Redis or in-memory for development)
export const cache = {
...
export default { db, query, initialize_database, seed_characters_if_empty, cache };

...
async function main() {
    console.log('⚔️  Generating IMPROVED Achilles v2 with textures and better geometry\n');

    try {
        // Step 1: Generate base 3D model
        console.log('Step 1: Generating base 3D model with improved prompts...');
        ...
        for (const [animName, actionId] of Object.entries(ANIMATIONS)) {
            ...
            const outputPath = path.join(__dirname, 'models', `achilles_v2_${animName}.glb`);
            await downloadFile(completedAnim.result.animation_glb_url, outputPath);
            console.log(`   💾 Saved: achilles_v2_${animName}.glb\n`);
        }

        console.log('🎉 SUCCESS! Achilles V2 with improved textures and animations complete!');
        console.log('\n📝 Improvements in V2:');
        console.log('  - Detailed facial features and skin texture');
        console.log('  - Bronze metallic armor coloring');
        console.log('  - Red cape and plume colors');
        console.log('  - Negative prompts to prevent strand artifacts');
        console.log('  - Clean separated helmet, plume, and cape elements');

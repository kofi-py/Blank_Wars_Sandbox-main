import { db, query } from './backend/src/database';
import { PreferencePopulationService } from './backend/src/services/preferencePopulationService';
import { PreferenceScoringService } from './backend/src/services/preferenceScoringService';
...
async function showExamplePreferences() {
    ...
    try {
        ...
        await client.query(
            `INSERT INTO characters (id, name, archetype, species, max_health, max_mana, max_energy, speed, attack, defense, special, description) 
       VALUES ($1, 'Gandalf the Test', 'mage', 'human', 100, 100, 100, 10, 10, 10, 10, 'A test mage') 
       ON CONFLICT (id) DO NOTHING`,
            [charId]
        );
        await client.query(
            `INSERT INTO user_characters (id, user_id, character_id, name, level) VALUES ($1, $2, $3, 'Gandalf the Test', 1) ON CONFLICT (id) DO NOTHING`,
            [charId, userId, charId]
        );

        // 2. Initialize Preferences (Mage)
        console.log('👉 Initializing Mage Preferences...');
        await PreferencePopulationService.initializeRankings(charId, 'mage');
        ...
        await PreferenceScoringService.refreshCharacterScores(charId);
        ...
        const rankings = await client.query(
            `SELECT category_type, category_value, rank, preference_score 
       FROM character_category_preferences 
       WHERE character_id = $1 
       ORDER BY category_type, rank DESC`,
            [charId]
        );
        ...
        const equipment = await client.query(
            `SELECT e.name, e.equipment_type, ce.preference_score 
       ...
       WHERE ce.character_id = $1`, [charId]
        ...
        const spells = await client.query(
            `SELECT sd.name, sd.category, cs.preference_score 

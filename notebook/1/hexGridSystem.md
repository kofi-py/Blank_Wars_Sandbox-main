...
export interface HexBattleGrid {
  gridSize: { rows: number; cols: number };  // 12x12
  characterPositions: Map<string, HexPosition>;
  terrain: Map<string, TerrainType>;
  perimeterAttempts: Map<string, number>;
  perimeterEffects: Map<string, PerimeterStatusEffect>;
}
...
export class HexGridSystem {
   ...
   */
  static toPixel(hex: HexPosition, hexSize: number): { x: number; y: number } {
   ...
   * Assumes flat-top hexagon orientation
   */
  static fromPixel(x: number, y: number, hexSize: number): HexPosition {
  ...
  static initializeBattleGrid(): HexBattleGrid {
    const grid: HexBattleGrid = {
      ...
      terrain: new Map(),
      perimeterAttempts: new Map(),
      perimeterEffects: new Map()
    };
  ...
  /**
   * Get starting positions for Team 1 (rows 1-3, near top)
   */
  static getTeam1StartPositions(): HexPosition[] {
    return [
      this.createHex(-2, 4),  // Left character
      this.createHex(0, 4),   // Center character
      this.createHex(2, 4)    // Right character
    ];
  }

  /**
   * Get starting positions for Team 2 (rows 10-12, near bottom)
   */

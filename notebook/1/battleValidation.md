...
export const BattleValidation = {
  ...
  validateCharacterHp(character: any, hp: number): number {
    const maxHp = character.maxHp || character.max_health || 100;
    return this.clamp(hp, 0, maxHp, 'currentHp');
  },
  ...
  validateCharacter(character: any): void {
    ...
    // Validate HP
    const currentHp = character.currentHp || character.current_health || 0;
    const maxHp = character.maxHp || character.max_health || 100;
    if (currentHp < 0 || currentHp > maxHp) {
      errors.push(`Invalid HP: ${currentHp}/${maxHp}`);
    }

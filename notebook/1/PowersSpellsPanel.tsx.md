...
export const PowersSpellsPanel: React.FC<PowersSpellsPanelProps> = ({
  ...
  const canUsePower = (power: PowerDefinition): { can: boolean; reason?: string } => {
    const cooldown = powerCooldowns[power.id] || 0;
    if (cooldown > 0) {
      return { can: false, reason: `Cooldown: ${cooldown} turns` };
    }

    const apCost = getAPCost(power.current_rank);
    if (currentAP < apCost) {
      return { can: false, reason: `Need ${apCost} AP` };
    }
  ...
  const canCastSpell = (spell: SpellDefinition): { can: boolean; reason?: string } => {
    ...
    const apCost = getAPCost(spell.current_rank);
    if (currentAP < apCost) {
      return { can: false, reason: `Need ${apCost} AP` };
    }

    if (currentMana < spell.mana_cost) {
  ...
  // Get rank indicator
  const renderRankIndicator = (currentRank: number, maxRank: number) => {
    return (
        ...
        {[...Array(maxRank)].map((_, i) => (
          <div
            ...
            className={`w-2 h-2 rounded-full ${
              i < currentRank ? 'bg-yellow-400' : 'bg-gray-600'
            }`}
          />
        ))}
      </div>
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4 mb-4">
                  ...
                  <div className="flex items-start justify-between mb-1">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <div className={`px-2 py-0.5 rounded text-xs font-bold ${
                        currentAP >= apCost ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'
                      }`}>
                        {apCost} AP
                      </div>
                  ...
                  <div className="flex items-start justify-between mb-1">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <div className="flex gap-1">
                        ...
                        <div className={`px-2 py-0.5 rounded text-xs font-bold ${
                          currentMana >= spell.mana_cost ? 'bg-blue-900 text-blue-300' : 'bg-red-900 text-red-300'
                        }`}>
                          {spell.mana_cost} MP
                        </div>
                      </div>
      ...
      {/* Empty State */}
      {equippedPowers.length === 0 && equippedSpells.length === 0 && (
